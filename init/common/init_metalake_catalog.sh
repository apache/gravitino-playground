...
if echo "$response" | grep -q "\"code\":0"; then
  ...
  response=$(curl -X POST -H "Content-Type: application/json" -d '{"name":"catalog_hive","type":"RELATIONAL", "provider":"hive", "comment":"comment","properties":{"metastore.uris":"thrift://'${HIVE_HOST_IP}':9083", "spark.bypass.spark.sql.hive.metastore.jars":"path", "spark.bypass.spark.sql.hive.metastore.jars.path":"file:///opt/spark/jars/*" }}' http://${GRAVITINO_HOST_IP}:${GRAVITINO_HOST_PORT}/api/metalakes/metalake_demo/catalogs)
  if echo "$response" | grep -q "\"code\":0"; then
    true # Placeholder, do nothing
  else
